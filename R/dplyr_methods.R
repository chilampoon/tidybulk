...
#'   used instead.
#' @param add.cell.ids from Seurat 3.0 A character vector of 
#'  length(x = c(x, y)). Appends the corresponding values to the 
#'  start of each objects' cell names.
#'
...
#'  See [group_by_drop_default()] for
#'  what the default value is for this argument.
#' @return A [grouped data frame][grouped_df()], unless the 
#'  combination of `...` and `add`
...
#' @param ...	Variables to be preserved when calling summarise(). 
#'  This is typically a set of variables whose combination uniquely 
#'  identify each row. NB: unlike group_by() you can not create new variables 
#'  here but instead you can select multiple variables with (e.g.) everything().
#'
...
#'  these suffixes will be added to the output to disambiguate them. Should be 
#'  a character vector of length 2. (See dplyr)
#' @param ... Data frames to combine (See dplyr)
#'
#' @return A tt object
#'
...
#' @param y tbls to join. (See dplyr)
#' @param by A character vector of variables to join by. (See dplyr)
#' @param copy If x and y are not from the same data source, and copy 
#'  is TRUE, then y will be copied into the same src as x. (See dplyr)
#' @param suffix If there are non-joined duplicate variables in x and y, 
#'  these suffixes will be added to the output to disambiguate them. 
#'  Should be a character vector of length 2. (See dplyr)
#' @param ... Data frames to combine (See dplyr)
#'
#' @return A tt object
#'
...
#'  TRUE, then y will be copied into the same src as x. (See dplyr)
#' @param suffix If there are non-joined duplicate variables in x and y, 
#'  these suffixes will be added to the output to disambiguate them. 
#'  Should be a character vector of length 2. (See dplyr)
#' @param ... Data frames to combine (See dplyr)
